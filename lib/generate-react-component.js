...
export default {
  ...
  close() {
    // Clear text input and error message, remove event listeners
    const panelChildren = this.modalPanel.getItem().children;

    panelChildren[1].getModel().setText('');
    panelChildren[2].textContent = '';

    Object.keys(panelChildren).slice(3).forEach(i => {
      panelChildren[i].removeEventListener('change', () => {
        this.options[panelChildren[i].id] = panelChildren[i].children[0].children[0].checked;
      });
    });
